<?php
require_once 'settings.php';  // include your DB settings file

// Redirect if accessed directly without POST
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: apply.php'); // Change to your form page URL
    exit();
}
...
// Street & suburb - max 40 characters
...
// Postcode - exactly 4 digits, and match state logic (simple example)
if (!preg_match("/^\d{4}$/", $postcode)) {
    $errors[] = "Postcode must be exactly 4 digits.";
} else {
    // Simple matching (example: VIC postcodes start with 3 or 8)
...
// Skills - at least one selected
if (!is_array($skills) || count($skills) == 0) {
    $errors[] = "Please select at least one technical skill.";
}

// Other skills - if any skills checked, other skills is optional, so no error needed

if (count($errors) > 0) {
    // Show errors and stop processing
...
// Assign skill fields (up to 4, fill null if fewer)
